...
@section('content')
    <div class="card">
        <div style="display:flex; justify-content:space-between; align-items:center; gap:1rem; flex-wrap:wrap; margin-bottom:0.5rem;">
            <h2>{{ __('admin.categories.heading', ['count' => $categories->count()]) }}</h2>
            <a href="{{ route('admin.categories.create') }}" class="btn btn-primary">{{ __('admin.categories.add') }}</a>
        </div>

        @if ($categories->isEmpty())
            <p class="empty">{{ __('admin.categories.empty') }}</p>
        @else
                ...
                <tbody>
                    @foreach ($categories as $category)
                        <tr>
                            ...
                            <td class="actions">
                                <a href="{{ route('admin.categories.edit', $category) }}" class="btn btn-secondary btn-sm">{{ __('admin.actions.edit') }}</a>
